...
你应该遇到过这种情况：拿到或接手一个项目，代码风格很好，结构很清晰，但你就是不知道从哪里开始看。找了半天，终于在某个文件夹里找到一个方法，点进去，是个接口，再找实现，在另一个文件夹。
...
一个函数要么是**统筹流程的主功能**（核心操作细节都在里面），要么是**干脏活的辅助功能**（不需要细读的工具函数），**不要混在一起**：
...
```python
# 筛选
valid_items = [item for item in items if item.price > 0 and item.stock > 0] # 把那些价格是0或者没货的都去掉

# 排序
sorted_books = sorted(books, key=lambda b: b.borrowCount, reverse=True) # 按借阅次数从多到少排，最热门的排第一

# 汇总
total = sum(order.price * order.qty for order in orders) # 把每个订单的单价乘以数量，然后全部加起来
```
...
同样的借书功能，UOP 只需要一个文件：给读者先定位，再看代码。
...
UOP 不是降低代码质量的借口，而是提高代码质量，防止写成史山的手段。判断一段代码是否达标：
...
如果你也在用 AI 辅助编程，想让它按 UOP 风格生成代码，可以直接把下面这段规则复制到你的 AI 编码助手的系统提示词或个人规则里（比如trae的“个人规则”，roo code的“自定义指令”等）。
...
提取出来的公共函数放在 utils 文件里，并注释说明它解决了什么通用问题、怎么调用。
```