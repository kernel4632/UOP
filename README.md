...
## 核心原则
把"任意背景的人能快速读懂"作为第一优先级。所有命名、结构、注释决策，都以这个标准为准。

## 命名规则
函数名使用"动词+名词"结构：add / make / get / find / show / list / update / remove 是首选动词。
布尔变量写成疑问句形式：isReady、hasPermission、canEdit、isLoggedIn。
变量名使用完整常见英语单词：result、user、manager、config、index。
id、api 等约定俗成的缩写保留。
大小写风格跟随所用语言惯例，可选情况下尽可能使用驼峰命名，易读性高。

## 函数规则
每个函数聚焦单一职责，长度保持在 4–10 行。
函数开头先用卫语句处理所有异常情况，每种情况直接 return，主逻辑写在最后。
缩进保持在两层以内。
一个函数要么做核心操作逻辑，要么做工具函数，两者分开写。
对同一对象操作的函数，放在一起，用类封装，简化函数命名，类名即对象名，调用可直接`object=class_name();object.method_name(…)`，如`book = Book();book.add("UOP", 3);book.borrow("UOP");`。

## 注释规则
每行有效代码后面跟一行大白话注释，解释这行在做什么事。
所有紧凑写法（列表推导、lambda、链式调用等）尾随注释解释这行在做什么事，哪些待参数可改。
注释写业务意图和背后原因，解释"为什么这么做"。
行为看起来奇怪的代码（限流等待、延迟处理等）加注释说明原因。
较长的函数用注释行做段落小标题，方便快速扫读。

## 文件规则
一个文件只负责一类业务：如user.py、book.py、order.py；report.js、utils.js 各管各的事。
文件数量在 4 个以内时，直接平铺放置，建立文件夹。
超过 3 个文件时，按业务大类做一层分组，嵌套层级保持在两层以内。
每个文件夹内放置至少3个文件，禁止出现一个文件的文件夹。

## 视觉规则
功能段落之间空 1–2 行，相关代码紧靠在一起，不相关的代码之间留空行隔开。
每个函数之间空两行，给阅读者视觉喘息空间。
重要步骤前加一行注释标题，说明这一步在做什么。
