...
(看见名字，不用跳转，不用猜，直接知道它是什么、在做什么。)

### 函数控制在 4–10 行，主次分明

包含空行和注释一起算。太短显得零碎，太长读起来累。
...
## 命名规则
只用最常见的简单英语：add / make / get / find / show / list / update / remove
用 result 不用 metrics，用 get 不用 fetch，宁可稍长也不让人猜。
函数名用"动词+名词"：sendEmail()、checkLogin()。
布尔值用疑问句：isReady、hasPermission。
不用 usr、mgr、cb 这类缩写。id、api 除外。

## 函数规则
每个函数控制在 4–10 行，包含空行和注释。
异常情况提前 return，不要层层嵌套。先把"不能做"的情况排掉，剩下的才是主逻辑。
缩进不超过两层。

## 注释规则
允许紧凑写法，但后面必须跟一句大白话注释解释它在做什么。
注释写"为什么"，不写"是什么"。x = x + 1 // 跳到下一页，这是好注释。// 把x加1，这是废话。
代码看起来很奇怪的地方，必须注释背后的原因，防止误删。

## 文件规则
一个文件只管一类事：user.py / book.py / order.py / report.py / utils.py。
文件名一眼知道里面是什么，不过度分层。

## 视觉规则
功能段落之间空 1–2 行。
重要步骤前加一行注释标题说明这一步在做什么。
不要把太多函数塞在同一个屏幕里。
```